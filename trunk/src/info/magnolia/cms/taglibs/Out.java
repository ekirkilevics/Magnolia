...
public class Out extends TagSupport {
		...
		if (contentNodeName!=null && !contentNodeName.equals("")) {
			...
			try {
				if (this.isNullOrEmtpy(contentNodeCollectionName)) {
					...
					this.setContentNode(actpage.getContentNode(contentNodeCollectionName).getContentNode(contentNodeName));
				}
			}
			catch (RepositoryException re) {log.debug(re.getMessage());}
		}
		else {
			if (local==null) {
				...
				else if ((contentNodeName!=null && contentNodeName.equals("")) || (contentNodeCollectionName!=null && contentNodeCollectionName.equals(""))) {
	...
	/**
	 * <p>set the requested node data</p>
	 *
	 * @param node
	 */
    ...
    /**
     * <p>set the node data name, e.g. "mainText"</p>
     *
     * @param name
     */
	...
	/**
	 * <p>set the content node name name, e.g. "01"</p>
	 *
	 * @param name
	 */
	...
	/**
	 * <p>set the content node collection name name, e.g. "mainColumnParagraphs"</p>
	 *
	 * @param name
	 */
    ...
    /**
    * <p>set which information of a file to retrieve </p>
	* <p>does only apply for nodeDatas of type=Binary</p>
	* <p>
    ...
    */
	...
	/**
	* <p>set which date format shall be delivered</p>
	* <p>does only apply for nodeDatas of type=Date</p>
	* <p>
	...
	* <p>set which date format shall be delivered</p>
	* <p>does only apply for nodeDatas of type=Date</p>
	* <p>language according to java.util.Locale</p>
	*
	* @param language
	*/
    ...
    /**
     ...
     private void display() {
         try {
			 /*
			 //@todo
			 //check if mutliple values (checkboxes) -> not nodeData but contentNode
			 try {
				 int i=0;
				 Iterator it=this.contentNode.getContentNode(this.nodeDataName).getChildren(ChildrenCollector.PROPERTY).iterator();
				 while (it.hasNext()) {
					 System.out.println(i++);
					 NodeData data=(NodeData) it.next();
					 System.out.println("GN:"+data.getName());
			...
			*/
			 ...
			 //if (this.property!=null) this.nodeData = this.contentNode.getContentNode(this.nodeDataName+"_properties").getNodeData(this.property);
			 ...
			 if (type==PropertyType.DATE) {
				 ...
				 if (this.getLineBreak().equals("")) value=nodeData.getString();
				 else value=nodeData.getString(this.getLineBreak());
			 }
	...
	public String getDateFormatted(Date date) {
		...
		if (lang==null) formatter = new SimpleDateFormat(this.getDatePattern());
		else formatter = new SimpleDateFormat(this.getDatePattern(),new Locale(lang));
	...
	public String getFilePropertyValue() {
		String value="";
		try {
			String fileProperty=this.getFileProperty();
			//@todo: contstants ...
			ContentNode props=this.getContentNode().getContentNode(this.nodeDataName+"_"+File.FILEPROPNAME_PROPERTIES);
			String filename=props.getNodeData(File.FILEPROPNAME_FILENAME).getString();
			String ext=props.getNodeData(File.FILEPROPNAME_EXTENSION).getString();
			...
			if (fileProperty.equals("extension")) {
				value=ext;
			}
			else if (fileProperty.equals("extensionLowerCase")) {
				value=ext.toLowerCase();
			}
			else if (fileProperty.equals("extensionUpperCase")) {
				value=ext.toUpperCase();
			}
			else if (fileProperty.equals("nameWithoutExtension")) {
				value=filename;
			}
			else if (fileProperty.equals("contentType")) {
				value=props.getNodeData(File.FILEPROPNAME_CONTENTTYPE).getString();
			}
			else if (fileProperty.equals("template")) {
				value=props.getNodeData(File.FILEPROPNAME_TEMPLATE).getString();
			}
			else if (fileProperty.equals("handle")) {
				value=this.getContentNode().getHandle()+"/"+this.getNodeDataName();
			}
			else if (fileProperty.equals("name")) {
				value=fullName;
			}
			else if (fileProperty.equals("pathWithoutName")) {
				value=this.getContentNode().getHandle()+"/"+this.getNodeDataName()+fullExt;
			}
			else if (fileProperty.equals("size")) {
				value=props.getNodeData(File.FILEPROPNAME_SIZE).getString();
			}
			else if (fileProperty.equals("sizeKB")) {
				double size=props.getNodeData(File.FILEPROPNAME_SIZE).getLong();
				String sizeStr;
				size=size/1024;
			...
			else if (fileProperty.equals("sizeMB")) {
				double size=props.getNodeData(File.FILEPROPNAME_SIZE).getLong();
				String sizeStr;
				size=size/(1024*1024);
				sizeStr=Double.toString(size);
			...
			else if (fileProperty.equals("sizeString")) {
				double size=props.getNodeData(File.FILEPROPNAME_SIZE).getLong();
				String unit="bytes";
				String sizeStr;
				if (size>=1000){
			 ...
			 else { //fileProperty.equals("path"|null|""|any other value")
				value=this.getContentNode().getHandle()+"/"+this.getNodeDataName()+"/"+fullName;
			}
	 	}

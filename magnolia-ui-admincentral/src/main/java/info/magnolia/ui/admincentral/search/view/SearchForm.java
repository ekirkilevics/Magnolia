...
/**
 * Simple form for searching. TODO: this is a very coarse implementation. Needs revision for layout,
 * naming and most of all it should be built dynamically, much like a dialog to allow for different
 * filters to be plugged in (think of advanced search) and combined with boolean ops in a
 * configurable manner.
 * @author dlipp
 * @author fgrilli
 * 
...
public class SearchForm extends Form implements Handler {
    ...
    public void updateUI(boolean searchResultsVisible, SearchResult result) {
        if (searchResultsVisible) {
            ...
            panel.addStyleName("m-search-form-expanded");
            if (result != null) {
                // TODO need to be i18nized
                ((SearchFormRow) expandedForm.getComponent(0)).getResultsArea().setValue(
                    result.getItemsFound() + " items found containing the text " + result.getQuery() + " in ");
            }
        }
        else {
            ...
            searchFormLabel.setVisible(false);
            panel.removeStyleName("m-search-form-expanded");
        }
    }
    ...
    /**
     * Represents a row in the search form. It is typically composed by a label, a filter (in the
     * form of a select) and a button for adding further filters. FIXME filter is hardcoded.
     * @author fgrilli
     * 
     */

...
public class OwfeJcrBeanCoder extends JcrBeanCoder {
    public OwfeJcrBeanCoder(final String ns, final Node startNode) {
        super(ns, startNode);
    }

    public OwfeJcrBeanCoder(final String ns, final Node startNode, final String beanNodeName) {
    ...
    protected Object[] asPropertyMapEntry(Object key, Object value) {
        if ((key instanceof StringAttribute) && isSafeForAnItemName(key.toString())) {
            if (value instanceof StringAttribute) {
                return new Object[]{key.toString(), value.toString()};
            }
        }

     ...
     protected synchronized String update(HttpServletRequest request) throws Exception {
         ...
         if (null != data) {
             ...
             try {
                 ...
                 // content might still exists under different uuid if it was autogenerated. Check the path first, if exists, then remove it before activating new content into same path
                 // TODO: handle same name siblings!
                 String path = newParentPath + (newParentPath.endsWith("/") ? "" : "/") + topContentElement.getAttributeValue(BaseSyndicatorImpl.RESOURCE_MAPPING_NAME_ATTRIBUTE);
                 if (hm.isExist(path)) {
                     hm.delete(path);
                 }
                 // new content

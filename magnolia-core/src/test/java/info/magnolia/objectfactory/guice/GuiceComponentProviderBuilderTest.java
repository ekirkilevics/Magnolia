...
public class GuiceComponentProviderBuilderTest extends AbstractMagnoliaTestCase {
    ...
    public void testBuildWithComponentConfigurers() {
        ...
        GuiceComponentProviderBuilder builder = new GuiceComponentProviderBuilder().withConfiguration(config);
        ComponentConfigurer configurer = mock(ComponentConfigurer.class);

        // WHEN
        builder.build(configurer);

        // THEN
        assertTrue(config.getConfigurers().contains(configurer));
    }

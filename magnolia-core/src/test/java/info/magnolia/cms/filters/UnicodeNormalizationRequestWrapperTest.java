...
public class UnicodeNormalizationRequestWrapperTest extends MgnlTestCase {
    ...
    public void testWillNotBlockNewlyAddedParametersOnForward() {
        ...
        mock.setupAddParameter("a", "12");

        UnicodeNormalizationRequestWrapper request = new UnicodeNormalizationRequestWrapper(mock);

        assertEquals("12", request.getParameter("a"));
        assertEquals(1, request.getParameterMap().size());
        assertNull(request.getParameter("b"));

        mock.setupAddParameter("b", "24");

        assertEquals("12", request.getParameter("a"));
        assertEquals(2, request.getParameterMap().size());
        assertEquals("24", request.getParameter("b"));
    }
}

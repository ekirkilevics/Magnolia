...
public class BackwardCompatibilityFilter extends OncePerRequestAbstractMgnlFilter {
    ...
    // private static final GlobalContentNode GLOBAL_CONTENT_NODE_DELEGATE = new GlobalContentNode();

    // private static final LocalContentNode LOCAL_CONTENT_NODE_DELEGATE = new LocalContentNode();

    private static final Actpage ACTPAGE_DELECATE = new Actpage();

    // private static final String GLOBAL_CONTENT_NODE_ATTRIBUTE = "contentObjGlobal";

    //private static final String LOCAL_CONTENT_NODE_ATTRIBUTE = "contentObj";
     ...
     * Logger.
     */
    private static Logger log = LoggerFactory.getLogger(BackwardCompatibilityFilter.class);
    
    private static class Actpage extends ContentDelegate{
        protected Content getWrappedContent() {
            printMessage(ACTPAGE_ATTRIBUTE, "cmsfn:mainPage()");
            return MgnlContext.getAggregationState().getCurrentContent();
        }
    }

    /*
    private static class GlobalContentNode extends ContentDelegate {

        protected Content getWrappedContent() {
            printMessage(GLOBAL_CONTENT_NODE_ATTRIBUTE,"cmsfn:globalParagraph()");
            return Resource.getGlobalContentNode();
        }
    }

    private static class LocalContentNode extends ContentDelegate {

        protected Content getWrappedContent() {
            printMessage(LOCAL_CONTENT_NODE_ATTRIBUTE, "cmsfn:currentParagraph()");
    ...
    */
    ...
    public void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        request.setAttribute(ACTPAGE_ATTRIBUTE, ACTPAGE_DELECATE);
        //request.setAttribute(LOCAL_CONTENT_NODE_ATTRIBUTE, LOCAL_CONTENT_NODE_DELEGATE);
        //request.setAttribute(GLOBAL_CONTENT_NODE_ATTRIBUTE, GLOBAL_CONTENT_NODE_DELEGATE);
        chain.doFilter(request, response);
    }
    
    private abstract static class ContentDelegate implements Content{
        
        protected abstract Content getWrappedContent();
        ...
        protected void printMessage(String oldAttributeName, String newFunction) {
            final String msg = "The request attribute ${" + oldAttributeName + "} is deprecated please use the new ${" + newFunction + "} function instead";
            if(log.isDebugEnabled()){
                log.debug(msg, new IllegalArgumentException());
            }
            else{
                log.warn(msg);
            }

...
public class UUIDLink {
    ...
    public static Pattern UUID_PATTERN = Pattern.compile("\\$\\{link:\\{uuid:\\{([^\\}]*)\\}," // the
                                                                                               ...
                                                                                               // the
                                                                                               // node
            + "repository:\\{([^\\}]*)\\}," + "(workspace:\\{[^\\}]*\\},)?" // is
                                                                            // not
                                                                            // supported
                                                                            // anymore
            + "(path|handle):\\{([^\\}]*)\\}" // fallback handle should not be
                                              // used unless the uuid is invalid
            + "(,nodeData:\\{([^\\}]*)\\}," // in case we point to a binary
                                            // (node data has no uuid!)
            + "extension:\\{([^\\}]*)\\})?" // the extension to be used in
                                            // rendering
            + "\\}\\}" // the handle
            + "(#([^\\?\"]*))?" // anchor
            + "(\\?([^\"]*))?"); // parameters

    protected static final Pattern LINK_PATTERN = Pattern.compile("(/[^\\.\"#\\?]*)" + // the
                                                                                       // handle
            "(\\.([\\w[^#\\?]]+))?" + // extension (if any)
            "(#([^\\?\"]*))?" + // anchor
            "(\\?([^\"]*))?" // parameters
    ...
    protected void init(String repository, String path, String extension, String anchor, String parameters) throws UUIDLinkException {
        ...
        try {
            ...
            if (node == null) {
                // this is a binary containing the name at the end this name is stored as an attribute but is not part of the handle
    ...
    public String toPattern() {
        return "${link:{" + "uuid:{" + getUUID() + "}," + "repository:{" + getRepository() + "}," + "handle:{" + getHandle() + "}," // original handle represented by the uuid
                ...
                + "}}" + (StringUtils.isNotEmpty(getAnchor()) ? "#" + getAnchor() : "") + (StringUtils.isNotEmpty(getParameters()) ? "?" + getParameters() : "");
    }

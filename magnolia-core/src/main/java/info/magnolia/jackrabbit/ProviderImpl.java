...
public final class ProviderImpl implements Provider {
	...
	private static final String sysRepositoryHome = System
			.getProperty("repository.home");

	private static final String sysRepositoryHomes = System
			.getProperty("repository.homes");

	/**
	 * Find the physical path to the repository folder
	 * 
	 * @param repositoryHome
	 *            the property set in the repository.xml file
	...
	public void init(RepositoryMapping repositoryMapping)
		...
		if (log.isInfoEnabled()) {
			log
					.info("Loading repository at " + repositoryHome + " (config file: " + configFile + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
		String contextFactoryClass = (String) params
				.get(CONTEXT_FACTORY_CLASS_KEY);
		...
		try {
			...
			try {
				...
				RegistryHelper.registerRepository(ctx, bindName, configFile,
						repositoryHome, true);
				this.repository = (Repository) ctx.lookup(bindName);
			}
			...
			public void execute(ServletContextEvent sce) {
				...
				try {
					...
					log.warn(MessageFormat.format(
							"Unable to shutdown repository {0}: {1} {2}",
							new Object[] { bindName, ne.getClass().getName(),
									ne.getMessage() }), ne);
				} catch (Throwable e) {
					log.warn(MessageFormat.format(
							"Failed to shutdown repository {0}: {1} {2}",
							new Object[] { bindName, e.getClass().getName(),
									e.getMessage() }), e);
				}
			}
		});
	 ...
	 */
	public Repository getUnderlineRepository()
			throws RepositoryNotInitializedException {
	 ...
	 */
	public void registerNamespace(String namespacePrefix, String uri,
		...
		try {
			...
			log
					.info("registering prefix [ " + namespacePrefix + " ] with uri " + uri); //$NON-NLS-1$ //$NON-NLS-2$
	...
	/**
	 * @see info.magnolia.repository.Provider#unregisterNamespace(java.lang.String,
	 *      javax.jcr.Workspace)
	 */
	public void unregisterNamespace(String prefix, Workspace workspace)
	 ...
	 */
	public void registerNodeTypes(Workspace workspace)
			throws RepositoryException {

		log.info("Registering node types");
		InputStream xml;
		String customNodeTypes = (String) this.repositoryMapping
				.getParameters().get(CUSTOM_NODETYPES);
		if (StringUtils.isEmpty(customNodeTypes)) {
			log
					.debug("No custom node type definition found, registering default magnolia node types");
			xml = getClass().getClassLoader().getResourceAsStream(
					MGNL_NODETYPES);
		} else {
			File nodeTypeDefinition = new File(Path
					.getAbsoluteFileSystemPath(customNodeTypes));
			try {
				xml = new FileInputStream(nodeTypeDefinition);
				log.info("Custom node types registered using : "
						+ customNodeTypes);
			} catch (FileNotFoundException e) {
				log.error("Unable to find node type definition : "
						+ customNodeTypes);
	...
	/**
	 * WORKAROUND for tomcat 5.0/jdk 1.5 problem tomcat\common\endorsed contains
	 * an xml-apis.jar needed by tomcat and loaded before all xmsl stuff present
	 * in the jdk (1.4 naming problem). In the xml-apis.jar file the
	 * TransformerFactoryImpl is set to
	 * "org.apache.xalan.processor.TransformerFactoryImpl" instead of
	 * "com.sun.org.apache.xalan.internal.xsltc.trax.TransformerFactoryImpl".
	 * solution: remove the file xml-apis.jar from the directory OR manually
	 * change the javax.xml.transform.TransformerFactory system property
	 */
	protected void checkXmlSettings() {
		if (SystemUtils.isJavaVersionAtLeast(1.5f)
			...
			log
					.info("Java 1.5 detected, setting system property \"javax.xml.transform.TransformerFactory\" to "
							+ "\"com.sun.org.apache.xalan.internal.xsltc.trax.TransformerFactoryImpl\"");
	...
	private void validateWorkspaces() throws RepositoryException {
		...
		WorkspaceImpl defaultWorkspace = (WorkspaceImpl) jcrSession
				.getWorkspace();
		String[] workspaceNames = defaultWorkspace

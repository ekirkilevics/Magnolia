...
public class MigrateFilterConfiguration extends BootstrapConditionally {
    ...
    private static final class CheckAndUpdateExistingFilters extends AllChildrenNodesOperation {
        ...
        protected void operateOnChildNode(Content node, InstallContext ctx) throws RepositoryException,
            ...
            try {
                ...
                final Filter30 originalFilter = (Filter30) filterChain30.get(currentFilter);
                ...
                if (originalFilter == null || 
                    ...
                    ctx.warn("Existing configuration of filter '" + currentFilter + "' has been modified or was not existing in original filter chain. Magnolia put a backup in " + FILTER_BACKUP_PATH + "/" + currentFilter + ". Please review the changes manually.");
                } 
                
                if (originalFilter != null && hasBypassChanged(originalFilter, existingFilter)) {
                    // TODO: transform the old bypasses to the new ones
                    ctx.info("Existing configuration of filter '" + currentFilter + "' has different bypass definitions. Magnolia put a backup in " + FILTER_BACKUP_PATH + "/" + currentFilter + ". Please review the changes manually.");
                }
        ...
        private boolean hasClassChanged(final Filter30 originalFilter, final Map existingFilter) {
            return !originalFilter.clazz.equals(existingFilter.get("class"));
        }
        
        private boolean hasBypassChanged(final Filter30 originalFilter, final Map existingFilter) {
        ...
        private boolean hasParamsChanged(final Filter30 originalFilter, final Map existingFilter) {
            final Map existingParameters = ((Map) existingFilter.get("params"));
            return !originalFilter.equalParams(existingParameters); 
        }
    
        private boolean hasPriorityChanged(final Filter30 originalFilter, final Map existingFilter) {

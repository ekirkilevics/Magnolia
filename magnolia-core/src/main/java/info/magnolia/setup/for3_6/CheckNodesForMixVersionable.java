            ...
            while (iter.hasNext()) {
                ...
                for (int i = 0; i < nt.length; i++) {
                    if ("mix:versionable".equals(nt[i].getName())) {
                        installContext.warn("There are nodes in your repository that contain unnecessary mix:versionable. Please replace all mix:versionable with mix:referencable supertype to achieve optimal repository performance. Refer to Magnolia Documentation at  http://documentation.magnolia.info/releases/3-6.html for details. An executable is provided for Enterprise Customers.");
                        return;
                    }
                }
            }

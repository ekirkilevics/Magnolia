...
public class JCRWorkItemStorage extends AbstractStorage {
	...
	private final static org.apache.log4j.Logger log = org.apache.log4j.Logger
			.getLogger(JCRWorkItemStorage.class.getName());
	...
	public List listWorkItems(String s, int limit) throws StoreException {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (true) {
					wi = new InFlowWorkItem();
					wi.setId(FlowExpressionId.fromParseableString(ct
							.getNodeData("ID").getString()));
					wi.setParticipantName(this.getName());
					// add attributes
					Collection attrs = ct.getNodeDataCollection();
					Iterator a_it = attrs.iterator();
					while (a_it.hasNext()) {
						NodeData nd = (NodeData) a_it.next();
						wi.addAttribute(nd.getName(), new StringAttribute(nd
								.getString()));
					}
				}
				*/

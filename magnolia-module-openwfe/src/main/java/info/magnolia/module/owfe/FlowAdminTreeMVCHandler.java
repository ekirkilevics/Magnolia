...
/**
 * this class wrapes the tree control. The AdminInterfaceServlet instantiates a
 * subclass. To build your own tree you have to override the prepareTree()
 * method
 * 
 * @author philipp
...
public abstract class FlowAdminTreeMVCHandler extends AdminTreeMVCHandler {
	...
	public FlowAdminTreeMVCHandler(String name, HttpServletRequest request,
		...
		this.setBrowseMode(StringUtils.equals(request
				.getParameter("browseMode"), "true"));
	}
	...
	public String getCommand() {
		...
		if (StringUtils.isNotEmpty(request.getParameter("treeAction"))) { //$NON-NLS-1$
			int treeAction = Integer.parseInt(request
					.getParameter("treeAction")); //$NON-NLS-1$
	...
	public String copy() {
		try {
			...
			AlertUtil.setMessage(MessagesManager
					.get(request, "tree.error.copy")
	 ...
	 */
	private void copyOrMove(int action) throws ExchangeException,
			RepositoryException {
		...
		newPath = getTree().pasteNode(pathClipboard, pathSelected, pasteType,
				action);
	...
	public String delete() {
		...
		try {
			...
			AlertUtil.setMessage(MessagesManager.get(request,
					"tree.error.delete")
					+ " " + AlertUtil.getExceptionMessage(e), request);
		}
		return VIEW_TREE;
	...
	public void launchActivateFlow(boolean recursive) {
		//boolean recursive = (request.getParameter("recursive") != null); //$NON-NLS-1$
		try {
			// ------- launch a flow to approve this activation
			log.info("###############################");
			PersistedEngine engine = OWFEEngine.getEngine();
			// AbstractEmbeddedParticipant ep = new
			// AbstractEmbeddedParticipant("developer"){
			//        		 
			// public void consume (WorkItem wi)
			// throws Exception
			// {
			// String strMap = "";
			// Iterator it = wi.getAttributes().alphaStringIterator();
			// while (it.hasNext()){
			// String attributeName = (String)it.next();
			// strMap += attributeName + "=" +
			// wi.getAttributes().get(attributeName).toString();
			// }
			// Log.trace("openwfe", "developer start consume workitem:" +
			// strMap);
			...
			//		          
			// };
			// ep.getParams().put("dispatcherClass",
			// "openwfe.org.engine.impl.dispatch.SocketDispatcher");
			// ep.getParams().put("host", "127.0.0.1");
			// ep.getParams().put("port", "7008");
			//        	 
			//        	 
			// engine.registerParticipant(ep);
			// ep = new AbstractEmbeddedParticipant("project-leader"){
			//        		 
			// public void consume (WorkItem wi)
			// throws Exception
			// {
			...
			// wi.getAttributes().get(attributeName).toString();
			// }
			// Log.trace("openwfe", "project-leader start consume workitem:" +
			// strMap);
			// //wi.getAttributes().puts("value", "CHF 10.-");
			//	                  
			// replyToEngine((InFlowWorkItem)wi);
			...
			// send mail
			//@ FIX it (send it asynch)
			MailHandler mh = new MailHandler("localhost", 1, 0);
			mh.setFrom("MagnoliaSystem");
			mh.setSubject("Workflow Request");
			mh.setToList("jackie_juju@hotmail.com");
			mh.setBody("----test---");
			mh.sendMail();
			
			
			// InMemoryWorklist wl =
			// (InMemoryWorklist)OWFEEngine.getEngine().getParticipant("project-leader");
			// wl.forward((InFlowWorkItem)wl.iterator().next());
			...
			AlertUtil
					.setMessage(
							"Your request has been dispatched, please wait for approval",
							request);
		} catch (Exception e) {
			log.error("can't launch activate flow", e);
			AlertUtil
					.setMessage("" + AlertUtil.getExceptionMessage(e), request);
		}
	...
	public String activate() {
		boolean recursive = (request.getParameter("recursive") != null); //$NON-NLS-1$
//		try{
//		Content c = getTree().getHierarchyManager().getContent(path);
//		c.lock(recursive, false);
//		}catch (Exception e){
//			Log.error(e);
//			AlertUtil.setMessage(MessagesManager.get(request,
//			"tree.error.activate")
...
//		}
		...
		launchActivateFlow(recursive);
		return VIEW_TREE;
	}

	public String _activate() {
		boolean recursive = (request.getParameter("recursive") != null); //$NON-NLS-1$
		// by default every CONTENTNODE under the specified CONTENT node is
		// activated
		try {
	...
	public String saveValue() {
		...
		String value = StringUtils.defaultString(request
				.getParameter("saveValue")); //$NON-NLS-1$
		...
		// if there was a displayValue passed show it instead of the written
		// value
		displayValue = StringUtils.defaultString(request
				.getParameter("displayValue"), value); //$NON-NLS-1$

		// @todo should be handled in a better way but, at the moment, this is
		// better than nothing
	 ...
	 * @param value
	 *            the new name
	 * @return return the new name (can change if there were not allowed
	 *         characters passed)
	 */
	...
	public void renderHtml(String view) throws IOException {
		...
		// an alert can happen if there were deactivation problems during a
		// renaming
		...
		if (VIEW_TREE.equals(view) || VIEW_CREATE.equals(view)
				|| VIEW_COPY_MOVE.equals(view)) {
			...
			if (view == VIEW_COPY_MOVE) {
				...
				// NOTE: tree.js checks for this pattern; adapt it there, if any
				// changes are made here
				html
						.append("<input type=\"hidden\" id=\"mgnlSelectNode\" value=\"" + newPath + "\" />"); //$NON-NLS-1$ //$NON-NLS-2$
			}
	...
	/**
	 * Create the html for the tree. Calls tree.getHtml after calling
	 * prepareTree.
	 * 
	 * @param html
	 */
	protected void renderTree(StringBuffer html) {
		String mode = StringUtils.defaultString(request
				.getParameter("treeMode")); //$NON-NLS-1$
		...
		if (!snippetMode) {
			...
			html
					.append("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>"); //$NON-NLS-1$
			renderHeaderIncludes(html);
			html.append("<title>Magnolia</title>"); //$NON-NLS-1$
			html.append("</head>"); //$NON-NLS-1$
			html
					.append("<body class=\"mgnlBgDark\" onload=\"" + tree.getJavascriptTree() + ".resizeOnload();\" >"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (!snippetMode) {
			html
					.append("<div id=\"" + tree.getJavascriptTree() + "_DivSuper\" style=\"display:block;\">"); //$NON-NLS-1$ //$NON-NLS-2$
		}

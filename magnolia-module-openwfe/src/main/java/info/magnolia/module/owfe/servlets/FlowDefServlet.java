...
/**
 * Servlet implementation class for Servlet: FlowDef This servlet is mainly for
 * testing (add flow def, list flow xml ...)
 */
public class FlowDefServlet extends javax.servlet.http.HttpServlet implements
	 ...
	 * name="webActivation" revision="1.0"><description
	 * language="default">worflow for web activation process.</description><sequence><participant
	 * ref="user-superuser" /><participant ref="command-activate" /></sequence></process-definition>
	 * 
	 */
	protected void doGet(HttpServletRequest request,
		...
		if (flowName == null) { // no parameter for name
			...
			// set start/end date
			sb.append("set date for node<br>" +
					"<form action=\"FlowDef\" method=\"get\">");
			sb
					.append("<input type=\"hidden\" name=\"method\" value=\"setDate\"/>"
			...
			// launch flow
			sb.append("launch flow<br>" + 
					"<form action=\"FlowDef\" method=\"get\">");
			sb
					.append("<input type=\"hidden\" name=\"method\" value=\"launchFlow\"/>"
							+ "path:<input name=\"path\" />"
			...
			sb.append("test<br>" +
					"<form action=\"FlowDef\" method=\"get\">");
			sb
					.append("<input type=\"hidden\" name=\"test\" value=\"yes\"/>parameter:<input name=\"testp\"></input>");
			sb.append("<br/>");
	...
	private String setDate(String pathSelected, String start, String stop)
			throws Exception {
		...
		hm.save();
		return "set date ok. path " + pathSelected + ", start date " + start
				+ ", stop date " + stop;
	}
}
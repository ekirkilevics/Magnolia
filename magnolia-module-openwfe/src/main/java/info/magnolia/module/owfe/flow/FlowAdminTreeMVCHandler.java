...
public abstract class FlowAdminTreeMVCHandler extends AdminTreeMVCHandler {
	...
	public String execute(String command) {
		...
		MgnlCommand tc = new CommandsMap().getCommand(command);
		if (tc == null) { // not found, do in the old ways
		//	tc = new CommandsMap().getFlowCommand(command);
		//	if (tc == null) {
				log.warn("can not find command named " + command
						+ "in tree command map");
	...
	//		}
		...
		params.put("recursive", Boolean.valueOf((request
				.getParameter("recursive") != null)));
	 ...
	 * @param vrequest
	 */
	protected abstract void prepareContextMenu(Tree vtree,

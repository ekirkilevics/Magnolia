...
public class Save {
     ...
     * Logger
     */
    public static Logger log = LoggerFactory.getLogger(Save.class);
    ...
    private String updateLinks(Content node, String name, String valueStr) throws AccessDeniedException,
        ...
        while (srcMatcher.find()) {
            ...
            src = StringUtils.replace(src, "../../../", request.getContextPath() + "/");
            ...
            if (uuidMatcher.find()) {
                ...
                Document doc = FCKEditorTmpFiles.getDocument(this.getRequest(), uuid);
                String fileNodeName = Path.getUniqueLabel(hm, filesNode.getHandle(), "file");
                Save.saveDocument(filesNode, doc, fileNodeName, "", "");
            ...
            else if (src.startsWith(request.getContextPath())) {
                link = pageName + StringUtils.removeStart(src, request.getContextPath() + this.getPath());
            }
    ...
    protected void processBinary(Content node, String name) throws PathNotFoundException, RepositoryException,
        ...
        if (doc == null && getForm().getParameter(name + "_" + File.REMOVE) != null) { //$NON-NLS-1$
            ...
            if (doc != null) {
                Save.saveDocument(node, doc, name, fileName, template);
            }
        }
    }

...
public class Tree extends ControlImpl {
    ...
    protected void getHtmlOfSingleItem(StringBuffer html, Content parentNode, String itemType, Object item) throws RepositoryException {
        ...
        if (itemType.equals(ITEM_TYPE_NODEDATA)) {
            ...
            if (d.isGranted(info.magnolia.cms.security.Permission.WRITE) && d.isMultiValue() != NodeData.MULTIVALUE_TRUE) {
        ...
        if (permissionWrite && StringUtils.isNotEmpty(htmlEdit)) {
            dblclick = " ondblclick=\"" + this.javascriptTree + ".editNodeData(this,'" + handle + "',0,'"+ StringUtils.replace(htmlEdit, "\"", "&quot;") + "');\""; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        }
        ...
        for (int i = 1; i < this.getColumns().size(); i++) {
            ...
            if (permissionWrite && StringUtils.isNotEmpty(htmlEdit)) {
                ...
                html.append(i);
                html.append(",'");
                html.append(StringUtils.replace(htmlEdit, "\"", "&quot;"));
                html.append("');\"");
                html.append(",'");

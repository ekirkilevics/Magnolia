...
CodePress = {
	...
	initialize : function() {
		...
		window.addEventListener('scroll', function() {
			if (!CodePress.scrolling)
				CodePress.syntaxHighlight('scroll')
		}, false);
	...
	keyHandler : function(evt) {
		...
		if ((evt.ctrlKey || evt.metaKey) && evt.shiftKey && charCode != 90) { // shortcuts
			// =
		...
		for ( var i = 0; i < complete.length; i++) {
			if (complete[i].input == trigger) {
				...
				parent.setTimeout(function() {
					CodePress.syntaxHighlight('complete', pattern, content)

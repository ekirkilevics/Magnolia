...
public class GroupEditRolesIncludeDialogPage extends SimplePageMVCHandler  {
    ...
    public GroupEditRolesIncludeDialogPage(String name, HttpServletRequest request, HttpServletResponse response) {
    ...
    protected void render(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            Content rolesOfGroup = group.getContent("roles"); //$NON-NLS-1$            
            Iterator it = rolesOfGroup.getChildren().iterator();
            while (it.hasNext()) {

...
public class ContentWriter {
    ...
    public ContentWriter(HierarchyManager hm, String repositoryName, String baseURL, HttpServletRequest request) {
        ...
        this.credentials = Authenticator.getCredentials(request);
        this.hierarchyManager = hm;
    }



    public void writeObject(String destination, SerializableContent content)
            throws RepositoryException {
        this.writeObject(this.hierarchyManager.getPage(destination), content);
    }
    ...
    public void writeObject(ContentNode destination, SerializableContentNode contentNode) {
    ...
    private void writeContent(Content parent, SerializableContent serializableContent)
        ...
        try {
            newPage = this.hierarchyManager.getPage(parentHandle+"/"+newPageName);
            this.safeDelete(newPage);
        } catch (PathNotFoundException e) {
            newPage = this.hierarchyManager.createPage(parent.getHandle(), newPageName);
        }

        try {
            /* write meta data */
            // todo
            this.writeMetaData(newPage,serializableContent.getMetaData());
            /* write all node data */
            this.writeNodeData(newPage, serializableContent);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
        /* write all content nodes */
        this.writeContentNode(newPage, serializableContent);
        ...
        while (contentIterator.hasNext()) {
            ...
            try {
                this.writeContent(newPage, sContent);
    ...
    private void removeNodedataList(Content node) {
        Collection subNodes = node.getChildren(ItemType.MAGNOLIA_NODE_DATA);
        if (subNodes.size() > 0) {
    ...
    private void removeContentNodeList(Content node) {
        Collection subNodes = node.getChildren(ItemType.MAGNOLIA_CONTENT_NODE);
        if (subNodes.size() > 0) {
            Iterator nodeIterator = subNodes.iterator();
            while(nodeIterator.hasNext()) {

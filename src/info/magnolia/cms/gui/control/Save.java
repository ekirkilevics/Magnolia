...
public class Save extends ControlSuper {
	...
	public void setForm(MultipartForm form) {this.form=form;}
	public MultipartForm getForm() {return this.form;}
	...
	public void save() {
		...
		try {
			...
			Content nodeCollection=new ContentNode();
			if (nodeCollectionName!=null) {
				try {
                    ...
                    System.out.println("PAGE - "+page.getHandle());
			...
			for (int i=0;i<saveInfo.length;i++) {
				...
				if (saveInfo[i].indexOf(",")!=-1) {
					...
					name=info[0];
					if (info.length>=2) type=PropertyType.valueFromName(info[1]);
					if (info.length>=3) valueType=Integer.valueOf(info[2]).intValue();
					if (info.length>=4) isRichEditValue=Integer.valueOf(info[3]).intValue();
					if (info.length>=5) encoding=Integer.valueOf(info[4]).intValue();
				}
				else {
					name=saveInfo[i];
				}
				...
				if (type==PropertyType.BINARY) {
					...
					if (doc==null && form.getParameter(name+"_"+File.FILEPROPNAME_REMOVE)!=null)	{
						//System.out.println("remove");
						try {
							node.deleteContentNode(name+"_"+File.FILEPROPNAME_PROPERTIES);
						}
						catch (RepositoryException re) {}
						try {
						...
						try {
							propNode=node.getContentNode(name+"_"+File.FILEPROPNAME_PROPERTIES);
						}
						catch (RepositoryException re) {
							try {
								if (doc!=null) propNode=node.createContentNode(name+"_"+File.FILEPROPNAME_PROPERTIES);
							}
						...
						if (propNode!=null) {
							...
							String fileName=form.getParameter(name+"_"+File.FILEPROPNAME_FILENAME);
							//System.out.println("fileName:"+fileName);
							if (fileName==null || fileName.equals("")) fileName=doc.getFileName();
							//System.out.println("fileName2:"+fileName);
							propData=propNode.getNodeData(File.FILEPROPNAME_FILENAME);
							if (!propData.isExist()) propData=propNode.createNodeData(File.FILEPROPNAME_FILENAME);
							...
							if (doc!=null) {
								...
								propData=propNode.getNodeData(File.FILEPROPNAME_CONTENTTYPE);
								if (!propData.isExist()) propData=propNode.createNodeData(File.FILEPROPNAME_CONTENTTYPE);
								propData.setValue(doc.getType());

								propData=propNode.getNodeData(File.FILEPROPNAME_SIZE);
								if (!propData.isExist()) propData=propNode.createNodeData(File.FILEPROPNAME_SIZE);
								propData.setValue(doc.getLength());

								propData=propNode.getNodeData(File.FILEPROPNAME_EXTENSION);
								if (!propData.isExist()) propData=propNode.createNodeData(File.FILEPROPNAME_EXTENSION);
								propData.setValue(doc.getExtension());

								String template=form.getParameter(name+"_"+File.FILEPROPNAME_TEMPLATE);
								if (template!=null && !template.equals("")) {
									propData=propNode.getNodeData(File.FILEPROPNAME_TEMPLATE);
									if (!propData.isExist()) propData=propNode.createNodeData(File.FILEPROPNAME_TEMPLATE);
									propData.setValue(template);
								}
								else {
									try {
										propNode.deleteNodeData(File.FILEPROPNAME_TEMPLATE);
									}
					...
					if (valueType==ControlSuper.VALUETYPE_MULTIPLE) {
						...
						String valueStr="";
						if (values!=null) valueStr=values[0]; //values is null when the expected field would not exis, e.g no checkbox selected
						NodeData data=node.getNodeData(name);

						if (isRichEditValue==1) valueStr=this.getRichEditValueStr(valueStr);
						...
						if (encoding==ControlSuper.ENCODING_BASE64) {
							...
							//no encoding
							if (values==null || valueStr.equals("")) remove=true;
							else write=true;
						}
						...
						if (remove) {
							...
							if (data.isExist()) node.deleteNodeData(name);
						}
						else if (write) {
							Value value=this.getValue(valueStr,type);
							//System.out.println("value: "+value);
							if (value!=null) {
								if (data.isExist()) data.setValue(value);
								else  node.createNodeData(name,value,type);
							}
						}
					}
				}
	...
	public Value getValue(String valueStr,int type) {
		...
		if (type==PropertyType.STRING) {
			...
			if (valueStr.equals("true")) value=new BooleanValue(true);
			else value=new BooleanValue(false);
		}
			...
			try {
				...
				try {
					...
					String[] dateAndTimeTokens = newDateAndTime.split(", ");
	...
	private String replaceABySpan(String value, String tagName) {
		...
		if (!tagName.equals(tagName.toUpperCase())) valueStr=this.replaceABySpan(valueStr,tagName.toUpperCase());
		return valueStr;
	}
	...
	private String replacePByBr(String value, String tagName) {
		...
		if (!tagName.equals(tagName.toUpperCase())) value=this.replacePByBr(value,tagName.toUpperCase());
		return value;
	}
	...
	private String replaceTag(String value, String before, String after) {
		...
		value=value.replaceAll("</"+before+">","</"+after+">");
		if (!before.equals(before.toUpperCase())) value=this.replaceTag(value,before.toUpperCase(),after);
		return value;
	}

...
public class Tree extends ControlSuper {
	 ...
	 */
	public void addItemType(String s) {this.getItemTypes().add(s);}
	...
	/**
	 * <p>set the double click event of the icon</p>
	 * @param s: javascriopt method
	 */
	public void setIconOndblclick(String s) {this.iconOndblclick=s;}
	public String getIconOndblclick() {return this.iconOndblclick;}
		...
		if (pasteType==PASTETYPE_SUB) {
			...
			try {
				...
				while (it2.hasNext()) {
					...
					if (c.getHandle().equals(selectedContent.getHandle())) selectedType=ItemType.NT_CONTENTNODE;
					if (c.getHandle().equals(touchedContent.getHandle())) touchedType=ItemType.NT_CONTENTNODE;
				}
				if (touchedType.equals(ItemType.NT_NODEDATA)) return ""; //sorting not possible
	...
	public String getHtml() {
		...
		if (!this.getSnippetMode()) html.append(this.getHtmlFooter());
	...
	public String getHtmlHeader() {
		...
		//resizer
		html.append("<div id=\""+this.getJavascriptTree()+"_ColumnResizerDiv\" style=\"position:absolute;top:-50px;z-index:500;\">");
		for (int i=1;i<this.getColumns().size();i++) {
			...
			html.append("<div onmousedown=\""+this.getJavascriptTree()+".dragColumnStart(this,"+i+");\" id=\""+this.getJavascriptTree()+"ColumnResizer"+i+"\" style=\"position:relative;left:-1000px;background-image:url("+this.getColumnResizer()+");display:inline;\">");
		...
		//column header
		html.append("<div id=\""+this.getJavascriptTree()+"_ColumnHeader\" style=\"position:absolute;top:-50px;z-index:480;\">");
		for (int i=0;i<this.getColumns().size();i++) {
			TreeColumn tc=this.getColumns(i);
			html.append("<span class=\""+this.getJavascriptTree()+"CssClassColumn"+i+"\"><span class=\"mgnlTreeHeader\"><nobr>"+tc.getTitle()+"</nobr></span></span>");
		}
		html.append("</div>");
		...
		html.append("<div id=\""+this.getJavascriptTree()+"_ColumnResizerLine\" style=\"position:absolute;top:0px;left:-100px;visibility:hidden;width:1px;height:"+this.getHeight()+"px;background-color:#333333;z-index:490;\"></div>");

		html.append("<div id=\""+this.getJavascriptTree()+"_"+this.getPath()+"_DivMain\" onclick=\""+this.getJavascriptTree()+".mainDivReset();\" oncontextmenu=\""+this.getJavascriptTree()+".menuShow(event);return false;\" class=\"mgnlTreeDiv\" style=\"height:"+this.getHeight()+"px;\">");
	...
	public String getHtmlFooter() {
		...
		try {
			...
			permissionWrite=root.isGranted(info.magnolia.cms.beans.runtime.Permission.ALL_PERMISSIONS);
		}
		catch (RepositoryException re) {}

		//line to place a very last position
		String lineId=this.getJavascriptTree()+"_"+this.getPath()+"_LineInter";
		html.append("<div id=\""+lineId+"\" class=\"mgnlTreeLineInter\"><img src=\""+Tree.LINE_INTERNODE_MASK+"\" width=\"1\" height=\"5\">");
		html.append("<img src=\"/admindocroot/0.gif\" width=\"150\" height=\"5\" onmouseover=\""+this.getJavascriptTree()+".moveNodeHighlightLine('"+lineId+"');\" onmouseout=\""+this.getJavascriptTree()+".moveNodeResetLine('"+lineId+"');\" onmousedown=\""+this.getJavascriptTree()+".pasteNode('"+this.getPath()+"',"+Tree.PASTETYPE_SUB+","+Boolean.toString(permissionWrite)+",'"+lineId+"');\">");
		html.append("</div>");

		html.append(new Hidden(this.getJavascriptTree()+"_"+this.getPath()+"_PermissionWrite",Boolean.toString(permissionWrite),false).getHtml());
		...
		html.append("<input id=\""+this.getJavascriptTree()+"_AddressBar\" type=\"text\" onkeydown=\"if (mgnlIsKeyEnter(event)) "+this.getJavascriptTree()+".expandNode(this.value);\" class=\"mgnlDialogControlEdit\" style=\"width:100%;\" value=\""+pathOpen+"\">");
		...
		if (this.getMenuItems().size()!=0) {
			...
			for (int i=0;i<this.getMenuItems().size();i++) {
				...
				if (item==null) {
					html.append("<div class=\"mgnlTreeMenuLine\"><img src=\"/admindocroot/0.gif\" width=\"1\" height=\"1\"></div>");
				}
				else {
					item.setJavascriptTree(this.getJavascriptTree());
					String id=this.getJavascriptTree()+"_MenuItem"+i;
					item.setId(id);
					menuJavascript.append(this.getJavascriptTree()+".menuItems["+counter+"]=new mgnlTreeMenuItem('"+id+"');\n");
					menuJavascript.append(this.getJavascriptTree()+".menuItems["+counter+"].conditions=new Object();");
					for (int cond=0;cond<item.getJavascriptConditions().size();cond++) {
						menuJavascript.append(this.getJavascriptTree()+".menuItems["+counter+"].conditions["+cond+"]="+item.getJavascriptCondition(cond)+";");
					}
		...
		html.append("<div id=\""+this.getJavascriptTree()+"_MoveShadow\" style=\"position:absolute;top:0px;left:0px;visibility:hidden;background-color:#ffffff;\"></div>");

		//"move denied"
		html.append("<img src=\""+Tree.DOCROOT+"move_denied.gif\" id=\""+this.getJavascriptTree()+"_MoveDenied\" style=\"position:absolute;top:0px;left:0px;visibility:hidden;\">");

		//initialize js tree object
		html.append("<script>");
		html.append("var "+this.getJavascriptTree()+"=new mgnlTree('"+this.getRepository()+"','"+this.getPath()+"','"+this.getJavascriptTree()+"',"+this.getHeight()+");");
		...
		for (int i=0;i<this.getColumns().size();i++) {
			...
			html.append(this.getJavascriptTree()+".columns["+i+"]=new mgnlTreeColumn("+tc.getWidth()+",'"+tc.getHtmlEdit()+"','"+tc.getName()+"',"+tc.getIsMeta()+","+tc.getIsLabel()+","+tc.getIsNodeDataValue()+","+tc.getIsNodeDataType()+");");
		}
	...
	public String getHtmlChildrenOfOneType(Content parentNode,String itemType) {
		...
		String icon;
		if (itemType.equals(ItemType.NT_CONTENT)) icon=this.getIconPage();
		else if (itemType.equals(ItemType.NT_CONTENTNODE)) icon=this.getIconContentNode();
		else if (itemType.equals(ItemType.NT_NODEDATA)) icon=this.getIconNodeData();
		else icon="";
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (itemType.equals(ItemType.NT_NODEDATA)) {
					...
					if (c.getAncestor(c.getLevel()-1).isGranted(info.magnolia.cms.beans.runtime.Permission.ALL_PERMISSIONS)) permissionWriteParent=true;
				...
				String jsHighlightNode=this.getJavascriptTree()+".nodeHighlight(this,'"+handle+"',"+Boolean.toString(permissionWrite)+");";
				String jsResetNode=this.getJavascriptTree()+".nodeReset(this,'"+handle+"');";
				String jsSelectNode=this.getJavascriptTree()+".selectNode('"+handle+"',"+Boolean.toString(permissionWrite)+",'"+itemType+"');";
				String jsExpandNode;
				if (this.getDrawShifter()) jsExpandNode=this.getJavascriptTree()+".expandNode('"+handle+"');";
				else jsExpandNode=jsSelectNode;

				String jsHighlightLine=this.getJavascriptTree()+".moveNodeHighlightLine('"+idPre+"_LineInter');";
				...
				if (permissionWriteParent) {
					html.append("<img src=\"/admindocroot/0.gif\" height=\"5\" width=\"150\" onmouseover=\""+jsHighlightLine+"\" onmouseout=\""+jsResetLine+"\" onmousedown=\""+this.getJavascriptTree()+".pasteNode('"+handle+"',"+Tree.PASTETYPE_ABOVE+",true);\">");
				}
				html.append("</div>");


				html.append("<div id=\""+idPre+"_DivMain\" style=\"position:relative;top:0;left:0;width:100%;height:18px;\">");
				html.append("&nbsp;"); //do not remove!

				int paddingLeft=left;
				if (paddingLeft<0) paddingLeft=0;
				html.append("<nobr><span id=\""+idPre+"_Column0Outer\" class="+this.getJavascriptTree()+"CssClassColumn0 style=\"padding-left:"+paddingLeft+"px;\">");
				...
				if (this.getDrawShifter()) {
					...
					if (!shifter.equals("")) html.append("<img id=\""+idPre+"_Shifter\" onmousedown=\""+this.getJavascriptTree()+".shifterDown('"+handle+"');\" onmouseout=\""+this.getJavascriptTree()+".shifterOut();\" class=mgnlTreeShifter src=\""+shifter+"\">");
				}
				html.append("<span id="+idPre+"_Name onmouseover=\""+jsHighlightNode+"\" onmouseout=\""+jsResetNode+"\" onmousedown=\""+jsSelectNode+this.getJavascriptTree()+".pasteNode('"+handle+"',"+Tree.PASTETYPE_SUB+","+permissionWrite+");\">");

				if (!icon.equals("")) {
					html.append("<img id=\""+idPre+"_Icon\" class=\"mgnlTreeIcon\" src=\""+icon+"\" onmousedown=\""+jsExpandNode+"\"");
					if (this.getIconOndblclick()!=null) html.append(" ondblclick=\""+this.getIconOndblclick()+"\"");
					html.append(">");
				}
				...
				for (int i=1;i<this.getColumns().size();i++) {
					...
					html.append("<nobr><span class=\""+this.getJavascriptTree()+"CssClassColumn"+i+"\"><span id=\""+idPre+"_Column"+i+"Main\""+tc.getHtmlCssClass()+tc.getHtmlEvents());
					if (permissionWrite && !tc.getHtmlEdit().equals("")) {
						html.append(" ondblclick=\""+this.getJavascriptTree()+".editNodeData(this,'"+handle+"',"+i+");\"");
					}

...
public class FileSrc extends TagSupport {
    ...
    public int doStartTag() {
        ...
        if (!this.contentNodeName.equals("")) {
            ...
            if (this.contentNode != null) this.contentNodeName = this.contentNode.getName();
            else {
                writeSrc("");
                return SKIP_BODY;
            }
        ...
        String contentNodeCollectionName = (String) pageContext.getAttribute(ContentNodeIterator.CONTENT_NODE_COLLECTION_NAME,PageContext.REQUEST_SCOPE);
		if (this.fileNameOnly.equals("true")) {
			...
			if (contentNodeCollectionName == null) {
				...
				try {
				writeSrc(Resource.getLocalContentNode(request).getHandle()+"/"+this.nodeDataName+this.slash+this.fileExtendedName);
				}
				catch (Exception e) {log.debug(e.getMessage());}
			}
		}
    ...
    private void setFileProperties() {
        ...
        if (contentNodeCollectionName == null) {
            // we are not in a loop
            try {
                properties = Resource.getGlobalContentNode(this.request).getContentNode(this.nodeDataName+"_properties");
            }
            catch (Exception e) {
            ...
            try {
                properties = Resource.getLocalContentNode(this.request).getContentNode(this.nodeDataName+"_properties");
            }
            catch (Exception e) {}
        }
        if (properties != null) {

...
public class RequestInterceptor extends HttpServlet {
    ...
    /**
     * <p>
     * Request and Response here is same as receivced by the original page
     * so it includes all post/get data
     * </p>
     * */
    ...
    /**
     * <p>
     * Sub action could be called from here
     * once this action finishes, it will continue loading the requested page
     * </p>
     *
     * */
    private void action() {
		...
		if (repository==null) repository=ContentRepository.WEBSITE;
		...
		if (action.equals("PREVIEW")) {
			...
			if (preview!=null) {
				if (preview.equals("true")) this.getRequest().getSession().setAttribute("mgnlPreview","true");
				else request.getSession().removeAttribute("mgnlPreview");
			}
		}
			...
			try {
				...
				if (seqPos0==0) {
					...
					nodeSelected.getMetaData().setSequencePosition(seqPos1-(MetaData.SEQUENCE_POS_COEFFICIENT*1000));
				}
				else if (seqPos1==0) {

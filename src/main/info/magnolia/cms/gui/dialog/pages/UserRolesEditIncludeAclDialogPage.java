...
public class UserRolesEditIncludeAclDialogPage extends BasePageServlet {
	...
	private static final String getHtmlRowInner(HttpServletRequest request,
			String dynamicTable, String repository) {
		boolean small = true;
		...
		accessRight.setOptions(msgs.get("roles.permission.readWrite"), Long
				.toString(PERMISSION_ALL));
		accessRight.setOptions(msgs.get("roles.permission.readOnly"), Long
				.toString(PERMISSION_READ));
		accessRight.setOptions(msgs.get("roles.permission.deny"), Long
				.toString(PERMISSION_NO));
		...
		if(!repository.equals(ContentRepository.USERS) && !repository.equals(ContentRepository.USER_ROLES)){
			html.append("<td width=\"1\" class=\""
				+ CssConstants.CSSCLASS_EDITWITHBUTTON + "\">"
				+ accessType.getHtml() + "</td>");
			html.append("<td width=\"1\">&nbsp;</td>");
		}
		
		html.append("<td width=\"100%\"class=\""
				+ CssConstants.CSSCLASS_EDITWITHBUTTON + "\">" + path.getHtml()
				+ "</td>");
		html.append("<td width=\"1\">&nbsp;</td>");
		html.append("<td width=\"1\" class=\""
				+ CssConstants.CSSCLASS_EDITWITHBUTTON + "\">"
	...
	/**
	 * @see info.magnolia.cms.servlets.BasePageServlet#draw(HttpServletRequest,
	 *      HttpServletResponse)
	 */
	 ...
	 * @throws IOException
	 */
	private void writeRepositoryTable(HttpServletRequest request, HttpServletResponse response, Messages msgs, PrintWriter out, Content role, String repository) throws RepositoryException, IOException {
		...
		// the table
		out.println("<table id=\"" + tableName + "\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"><tr><td></td></tr></table>");
		
		// add button
		
		out.println("<table width=\"100%\">");
		DialogButton add = DialogFactory.getDialogButtonInstance(
		...
		out.println( dynamicTableName + " = new MgnlDynamicTable('"+tableName+"',document.mgnlFormMain."+ hiddenFieldName +", aclGetNewPermissionObject, aclGetPermissionObject, acl"+repository+"RenderFunction, null);");
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (skipNext) {
					...
					String accessRight = c.getNodeData("permissions")
							.getString();
					String accessType;
					...
					out.println(dynamicTableName + ".add({accessRight:"+accessRight+",accessType:'"+accessType+"',path:'"+path+"'});");
				}
			}
		} catch (Exception e) {
			noAcl = true;

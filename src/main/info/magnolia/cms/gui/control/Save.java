...
public class Save extends ControlSuper {
				...
				if (type==PropertyType.BINARY) {
					...
					if (doc==null && form.getParameter(name+"_"+File.REMOVE)!=null)	{
						...
						try {
							...
							try {
								if (doc!=null) propNode=node.createContentNode(name+"_"+FileProperties.PROPERTIES_CONTENTNODE);
							}
						...
						if (propNode!=null) {
							...
							if (!propData.isExist()) propData=propNode.createNodeData(FileProperties.PROPERTY_FILENAME);
							...
							if (doc!=null) {
								...
								if (!propData.isExist()) propData=propNode.createNodeData(FileProperties.PROPERTY_CONTENTTYPE);
								propData.setValue(doc.getType());

								propData=propNode.getNodeData(FileProperties.PROPERTY_SIZE);
								if (!propData.isExist()) propData=propNode.createNodeData(FileProperties.PROPERTY_SIZE);
								propData.setValue(doc.getLength());

								propData=propNode.getNodeData(FileProperties.PROPERTY_EXTENSION);
								if (!propData.isExist()) propData=propNode.createNodeData(FileProperties.PROPERTY_EXTENSION);
								...
								if (template!=null && !template.equals("")) {
									...
									if (!propData.isExist()) propData=propNode.createNodeData(FileProperties.PROPERTY_TEMPLATE);
	...
	private String replaceABySpan(String value, String tagName) {
		...
		if (!tagName.equals(tagName.toUpperCase())) valueStr2=this.replaceABySpan(valueStr2,tagName.toUpperCase());
		return valueStr2;
	}

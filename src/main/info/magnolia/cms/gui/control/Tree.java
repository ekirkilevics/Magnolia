		...
		try {
			...
			while (it.hasNext()) {
				...
				if (itemType.equals(ItemType.NT_NODEDATA)) {
					...
					handle=d.getHandle();
					name=d.getName();
					if (d.isGranted(info.magnolia.cms.security.Permission.WRITE)) permissionWrite=true;
				}
				else {
					c=(Content) o;
					handle=c.getHandle();
					...
					if (c.isGranted(info.magnolia.cms.security.Permission.WRITE)) permissionWrite=true;
					if (c.getAncestor(c.getLevel()-1).isGranted(info.magnolia.cms.security.Permission.WRITE)) permissionWriteParent=true;

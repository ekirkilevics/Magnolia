...
/*
 * import org.apache.commons.id.IdentifierUtils; import org.apache.commons.codec.DecoderException; import
 * org.apache.commons.codec.binary.Hex; import org.apache.commons.codec.digest.DigestUtils;
 */
/** XXX end modification by stefan@apache.org */
/**
 * <p>
 * <code>UUID</code> represents a Universally Unique Identifier per IETF Draft specification. For more information
 * regarding the IETF Draft UUID specification
 * </p>
 * <p>
...
public class UUID implements Constants, Serializable, Comparable {
     ...
     * </p>
     * <p>
     * Method is protected as their is no standard as to the internal representation of a UUID. In this case a
     * BigInteger is used with signum always positive.
     * </p>
     ...
     */
    /*
     * protected UUID(BigInteger bigIntValue) throws IllegalArgumentException { super(); if (bigIntValue.bitLength() >
     * UUID.UUID_BIT_LENGTH) { throw new IllegalArgumentException("UUID must be contructed using a 128 bit BigInteger"); }
     ...
     * </p>
     * @param uuidString the UUID formatted String to parse. XXX begin modification by stefan@apache.org XXX end
     * modification by stefan@apache.org
    ...
    public static UUID fromString(String uuidString) {
        ...
        /** XXX begin modification by stefan@apache.org */
        /*
         * try { char[] chars = buf.toString().toCharArray(); tmpUUID = new UUID(Hex.decodeHex(chars)); } catch
    ...
    public String toString() {
        ...
        if (stringValue == null) {
            /** XXX begin modification by stefan@apache.org */
            /*
             * StringBuffer buf = new StringBuffer(new String(Hex.encodeHex(rawBytes))); while (buf.length() !=
             * UUID_UNFORMATTED_LENGTH) { buf.insert(0, "0"); } buf.ensureCapacity(UUID_FORMATTED_LENGTH);
    ...
    /**
     * <p>
     * Returns a urn representation of the UUID. This is same as the toString() value prefixed with
     * <code>urn:uuid:</code>
     * </p>
    ...
    /**
     * <p>
     * Returns the clock sequence value in the UUID. The clock sequence is a random assigned to a particular clock
     * instance that generated the time in the timestamp of a time based UUID.
     * </p>
     ...
     * 802 MAC address. The specification permits this value to be calculated from other sources other than the MAC.
     * </p>
    ...
    /**
     * <p>
     * Returns the timestamp value of the UUID as 100-nano second intervals since the Gregorian change offset
     ...
     */
    /*
     * static UUID timeUUID() { return (UUID) IdentifierUtils.UUID_VERSION_ONE_GENERATOR.nextIdentifier(); }
     */
    /** XXX end modification by stefan@apache.org */
    /** XXX begin modification by stefan@apache.org */
    /**
     ...
     */
    /*
     * static UUID nameUUIDFromString(String name, UUID namespace) { byte[] nameAsBytes = name.getBytes(); byte[] concat =
     * new byte[UUID_BYTE_LENGTH + nameAsBytes.length]; System.arraycopy(namespace.getRawBytes(), 0, concat, 0,
     * UUID_BYTE_LENGTH); System.arraycopy(nameAsBytes, 0, concat, UUID_BYTE_LENGTH, nameAsBytes.length); byte[] raw =
     * DigestUtils.md5(concat); //Set version raw[TIME_HI_AND_VERSION_BYTE_6] &= 0x0F; raw[TIME_HI_AND_VERSION_BYTE_6] |=
     * (UUID.VERSION_THREE < < 4); //Set variant raw[CLOCK_SEQ_HI_AND_RESERVED_BYTE_8] &= 0x3F; //0011 1111
     * raw[CLOCK_SEQ_HI_AND_RESERVED_BYTE_8] |= 0x80; //1000 0000 return new UUID(raw); }
     */
    /** XXX end modification by stefan@apache.org */
}

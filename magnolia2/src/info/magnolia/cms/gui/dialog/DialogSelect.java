...
public class DialogSelect extends DialogBox {
	...
	public DialogSelect (ContentNode configNode,Content websiteNode) {
	...
	public void setOptions(ContentNode configNode) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				String label=null;
				if (n.getNodeData("label").isExist()) label=n.getNodeData("label").getString();
				SelectOption option=new SelectOption(label,value);
				if (n.getNodeData("selected").getBoolean()==true) option.setSelected(true);
				options.add(option);
			}
		}
